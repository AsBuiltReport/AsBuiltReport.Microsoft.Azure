                    ...
                    Section -Style Heading4 'Virtual Networks' {
                        ...
                        foreach ($AzVirtualNetwork in $AzVirtualNetworks) {
                            $InObj = [Ordered]@{
                                ...
                                'Address Space' = if ($AzVirtualNetwork.AddressSpace.AddressPrefixes) {
                                    $AzVirtualNetwork.AddressSpace.AddressPrefixes -join ', '
                                } else {
                                    'Unknown'
                                }
                                'DNS Servers' = if ($AzVirtualNetwork.DhcpOptions.DnsServers) {
                            ...
                            if ($Options.ShowTags) {
                                $InObj['Tags'] = if ([string]::IsNullOrEmpty($AzVirtualNetwork.Tag)) {
                                    'None'
                                } else {
