...
Describe 'Localization Data Consistency Tests' {
    Context 'MicrosoftAzure.psd1 Localization Files' {
        BeforeAll {
            $TemplateFile = Join-Path -Path $LanguagePath -ChildPath 'en-US\MicrosoftAzure.psd1'
        ...
        foreach ($folder in (Get-ChildItem -Path (Join-Path -Path $PSScriptRoot -ChildPath '..\AsBuiltReport.Microsoft.Azure\Language') -Directory | Where-Object { $_.Name -ne 'en-US' })) {
            It "Language '<Name>' should have all keys from en-US template for MicrosoftAzure.psd1" -TestCases @(@{ Name = $folder.Name; FolderPath = $folder.FullName }) {
                param($Name, $FolderPath)

                $LocalizedFile = Join-Path -Path $FolderPath -ChildPath 'MicrosoftAzure.psd1'
                if (Test-Path $LocalizedFile) {
                    $LocalizedKeys = Get-NestedLocalizationKeys -FilePath $LocalizedFile
                    $TemplatePath = Join-Path -Path $LanguagePath -ChildPath 'en-US\MicrosoftAzure.psd1'
